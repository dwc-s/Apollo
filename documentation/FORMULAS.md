...
| Precision consistency (trend) | `_report_precision_consistency` (+ `_precision_consistency_multi`) | per-**day** R95 (`_ap_series(..., by='day')`) computed **per (face × distance) combination** — R95 is size-normalized but not distance-normalized, so pooling different distances/faces mixes genuinely different precision. ≥2 combos with 3+ qualifying days → a multi-line overlay, one smoothed (`_PREC_CONSISTENCY_WINDOW`=5-day MA) trace each; exactly one → the classic single line + ±1 within-window-stdev band (ddof=1, clamped ≥0) + raw dots + least-squares trend; none rich enough → a pooled fallback over the (optionally face/distance-filtered) selection. `_ap_series` takes optional `face_filter` / `distance_filter` sets (empty = all). Responsive counterpart to the *cumulative* all-time R95 trace. |
